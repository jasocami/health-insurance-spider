...
from apps.core_records.models import ClinicPointRecord, QuironRecord, SaludonnetRecord, SmartSalusRecord, \
    IglobalmedRecord, OperarmeRecord, MiDiagnosticoRecord, BonomedicoRecord, PortalsaludSanitasRecord


class CSVDownloadView:
    """
    Base class for CSV downloads.
    """
    ...
    def create_tmp_dir(self):
        """
        Creates the temporary folder for storing files if it does not exist.
        """
    ...
    def write(self):
        """
        Must be overridden by all subclasses. Responsible for writing the CSV file to disk.
        """
...
class QuironRecordsCSVDownloadView(CSVDownloadView):
    def write(self):
        ...
        with open(path, 'w', encoding='utf-8') as csv_file:
            writer = csv.writer(
                csv_file, dialect='excel', delimiter=self.CSV_DELIMITER, quotechar='"', quoting=csv.QUOTE_ALL
            )
            writer.writerow([
                'tipo_producto', 'nombre_producto', 'especialidad', 'pvp',
                'grupo', 'provincia', 'centro', 'descripcion',
                'url', 'fecha_creacion',
            ])

            for rec in records:
                row = [
                    rec.type_product_name, rec.product_name, rec.speciality_name, rec.pvp, rec.group, rec.province_name,
                    rec.center, rec.description, rec.url, rec.creation_timestamp
                ]
                writer.writerow(row)
...
class SaludOnNetRecordsCSVDownloadView(CSVDownloadView):
    def write(self):
        ...
        with open(path, 'w', encoding='utf-8') as csv_file:
            writer = csv.writer(
                csv_file, dialect='excel', delimiter=self.CSV_DELIMITER,
