...
from apps.spider_hub.spiders.base_spider import BaseScrape
from apps.core_records.models import QuironRecord as Record
...
class QuironScrape(BaseScrape):
    ...
    def __parse_provinces(self, response, **kwargs):
        ...
        :return:
        """
        print(f'Parse provinces start {response.request.url}')
        ...
        for province in rs_json:
            ...
            print(f'Parse provinces province {province_name}')
    ...
    def __parse_product(self, response, **kwargs):
        ...
        :return:
        """
        print(f'Parse product start {response.request.url}')
    ...
    def __parse_product_physic(self, response, **kwargs):
        ...
        :return:
        """
        print(f'Parse product physic start {response.request.url}')
    ...
    def __parse_centers_for_province(self, response, **kwargs):
        ...
        :return:
        """
        print(f'Parse centers for province {response.request.url}')
        ...
        for center in rs_json:
            ...
            try:
                ...
                print(f'Object {obj} | is new {created}')
            except Exception as e:
                print(f"Error for url {kwargs['url']} || {e}")

    def __get_provinces_url(self, speciality_value_id, speciality_attr_id):
        return (
            f'https://e-quironsalud.es/?module=citacion&fc=module&controller=datos'
            f'&action=GetProvinciasBuscador&productId={speciality_value_id}'
            f'&idProductAttribute={speciality_attr_id}'
        )

    def __get_product_url(self, speciality_value_id, speciality_attr_id, province_name=''):
        return (
            f'https://e-quironsalud.es/index.php?id_product={speciality_value_id}'
            f'&id_product_attribute={speciality_attr_id}&controller=product'
        )

    def __get_product_action_kind_url(self, token, id_product, group_id, product_id_list):
        return (
            'https://e-quironsalud.es/index.php?controller=product'
            '?token={}&id_product={}&id_customization=0&group[5]={}{}'
        ).format(
            token,
            id_product,
            group_id,
    ...
    def __get_centers_from_province_url(self, province_id, product_id, id_product_attr):
        return (
            f'https://e-quironsalud.es/index.php?module=citacion&fc=module&controller=datos'
            f'&action=CitaProvincia&provinciaId={province_id}'
            f'&productId={product_id}&idProductAttribute={id_product_attr}'
        )
