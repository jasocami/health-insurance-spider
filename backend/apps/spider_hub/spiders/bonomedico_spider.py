...
from apps.core_records.models import BonomedicoRecord as Record
...
class BonomedicoScrape(BaseScrape):
    ...
    def __parse_specialities_page(self, response, **kwargs):
        ...
        for option in options:
            for link in option.css('ul a'):
                ...
                print(f'Parse province {pro_name}, speciality {name}')
                yield scrapy.Request(
                    url=href,
    ...
    def __parse_services_list(self, response, **kwargs):
        ...
        print(f'Parse services list start {response.request.url}')
    ...
    def __parse_service(self, response, **kwargs):
        ...
        if product_name:
            ...
            for items in product_name.css('tr'):
                ...
                raw_price = items[1].get().replace('\n', '').replace('\t', '').replace('€', '').strip()
                price = BeautifulSoup(raw_price, "lxml").text.strip()
                if price:
                    try:
        ...
        if center_name:
            ...
            for row in center_name:
                ...
                for center in row.css('.listado-centros-donde li'):
                    c_name = f'{rol}:{center.css("a::text").get()}'
