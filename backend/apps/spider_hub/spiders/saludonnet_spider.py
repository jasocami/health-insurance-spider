...
from apps.core_records.models import SaludonnetRecord as Record
...
class SaludOnNetScrape(BaseScrape):
    ...
    def __parse_main_page(self, response, **kwargs):
        ...
        for option in specialities_options:
            ...
            for province in kwargs['provinces_list']:
                ...
                print(f'Parse services for province {p_name}, speciality {spe_name}')
    ...
    def __parse_services(self, response, **kwargs):
        ...
        :return:
        """
        print(f'Parse services API start {response.request.url}')
    ...
    def __parse_service(self, response, **kwargs):
        ...
        :return:
        """
        print(f'Parse service start {response.request.url}')
        ...
        pvp_full = (
            (str(kwargs.get("pvp_full")) or response.css('span#productPriceDesktop::text').get()) or ''
        ).replace('€', '').strip()[:64]
        group = 'Consulta'
        description = response.css(
            'div.l-content-wrapper div.content-wrapper .tabs-wrapper'
            ' #tab-container #tabGeneral .list'
        )[0].get()
        ...
        try:
            health_registration = (
                response.css('div.product__header__filter .product-header__health-registration::text')
                .get().split(':')[-1].strip()
            )
        except Exception:
            health_registration = ''
        ...
        try:
            ...
            print(f'Error for url {response.request.url} || {e}')

    def __get_services_url(self, speciality_name, province_id):
        return f'https://www.saludonnet.com/servicios-medicos/{speciality_name}/provincia/{province_id}'

    def __get_centers_api(self, health_service, speciality, province, page):
        return (
            f"https://www.saludonnet.com/sin-seguro-medico/api/v0/find/services"
            f"?healthService={health_service}"
            f"&specialty={speciality}"
            f"&province={province}"
            f"&city=&isSearchingByStreet=False&lat=0&lon=0&radiusInKm=0"
            f"&userLatitude=0.4166&userLongitude=-0.7003&resultsPerPage=30&keyword=&orderBy=price&page={page}"
        )
