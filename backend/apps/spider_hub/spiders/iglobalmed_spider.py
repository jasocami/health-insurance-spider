...
from apps.core_records.models import IglobalmedRecord as Record
...
class IglobalmedScrape(BaseScrape):
    ...
    def __parse_specialities_page(self, response, **kwargs):
        ...
        for option in specialities_options:
            ...
            print(f'Parse specialities for city {city_name}, speciality {spe_name}')
    ...
    def __parse_services_page(self, response, **kwargs):
        ...
        print(f"Parse services page for city {kwargs['city_name']}")
    ...
    def __parse_service(self, response, **kwargs):
        ...
        pvp_full = response.css(
            '#product-box #product-box-info #product-box-info-checkout'
            ' #product-box-info-checkout-product-price .price'
        ).get()
        if pvp_full:
        ...
        pvp_consultation = response.css(
            '#product-box #product-box-info #product-box-info-checkout'
            ' #product-box-info-checkout-price'
        ).get()
        if pvp_consultation:
        ...
        try:
            pvp = response.css(
                '#product-box #product-box-info #product-box-info-checkout'
                ' #product-box-info-checkout-reserva small'
            ).get().lower().replace('\n', '').strip()
