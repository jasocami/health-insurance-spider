...
from apps.core_records.models import OperarmeRecord as Record
...
class OperarmeScrape(BaseScrape):
    ...
    def __parse_pre_service(self, response, **kwargs):
        ...
        print(f'Parse pre service start {response.request.url}')
    ...
    def __parse_service(self, response, **kwargs):
        ...
        if not center_name:
            centers = response.css(
                '#banner-medical-check-offer #content-banner-center'
                ' .item-wrapper-selector .item-center'
            )
            center_name = []
            address = []
            for item in centers:
