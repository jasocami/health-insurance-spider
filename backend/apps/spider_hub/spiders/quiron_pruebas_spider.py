...
class QuironScrape(BaseScrape):
    ...
    def parse(self, response, **kwargs):
        ...
        by_image_items = response.css(
            'nav#cbp-hrmenu div.menu-element-id-29 div.menu_row'
            ' div.cbp-menu-column-inner div.col-12'
        )
        tests_items = response.css(
            'nav#cbp-hrmenu div.menu-element-id-39 div.menu_row'
            ' div.cbp-menu-column-inner div.col-12'
        )
    ...
    def __parse_product(self, response, **kwargs):
        ...
        pvp = (
            response.css('div.product-prices span.product-price::text')
    ...
    def __parse_centers_for_province(self, response, **kwargs):
        ...
        :return:
        """
        print(f'Parse centers for province start {response.request.url}')
    ...
    def __get_centers_from_province_url(self, url, province_id, product_id):
        return (
            f'{url}?module=citacion&fc=module&controller=datos'
            f'&action=CitaProvinciaOtros&provinciaId={province_id}&productId={product_id}'
        )
